...
{	
	...
	//test markers (controllo con stampa cout)
	TestMarkersValidity(mesh);
	
	//test non-zero lenght
	if (!TestLengthValidity(mesh))
	{
    cerr << "Errore nei test su length dei lati" << endl;
    return 1;	
	}
	
	//test non-zero area
	if (!TestAreaValidity(mesh))
	{
    cerr << "Errore nei test su area dei poligoni" << endl;
    return 1;	
	}
